import { m as n, T as l, d as c, C as u } from "./index.17583699.js";
window.Alpine = n;
n.data("SetupForm", () => ({
  init() {
    this.$root.querySelectorAll("input").forEach(e => {
      e.addEventListener("keypress", t => {
        t.key == "Enter" &&
          (t.preventDefault(),
          t.target.closest(".tab-pane").querySelector("button[data-href]").click());
      }),
      ...
      window.addEventListener("storage", function (e) {
        e.key == "integrations" &&
          ...
          ($("#integration-mlc").text("Already Configured").attr("disabled", !0),
          window.focus(),
  ...
  validateFileSize(e, t) {
    e.target.files[0].size > t &&
      (confirm(
        `This image file is larger than ${t / 1e3}KB which may result in increased load times. Are you sure you'd like to use this file?`,
      ) ||
        (e.target.value = ""));
  },
  switchTab(e) {
  ...
  mlcSetup() {
    let e = u.config.urlRoot,
      t = {
        ...
        integration_setup_url: window.location.origin + e + "/setup/integrations",
    ...
    const a = [];
    for (let r in t) a.push(encodeURIComponent(r) + "=" + encodeURIComponent(t[r]));
    window.open("https://www.majorleaguecyber.org/events/new?" + a.join("&"), "_blank");
  },
  submitSetup(e) {
    if (document.querySelector("#newsletter-checkbox").checked) {
      ...
      const o = [];
      for (let i in r) o.push(encodeURIComponent(i) + "=" + encodeURIComponent(r[i]));
