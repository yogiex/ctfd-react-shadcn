...
import { AlertCircle, Trophy, Award, ExternalLink } from 'lucide-react'
...
export function UserPublicProfile() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get<Solve[]>(`/users/${id}/solves`).catch(() => [] as Solve[]),
      api.get<AwardItem[]>(`/users/${id}/awards`).catch(() => [] as AwardItem[]),
    ])
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {user.bracket_name && (
                  <Badge variant="secondary">{user.bracket_name}</Badge>
                )}
              </div>
            ...
            <div className="text-right shrink-0">
              <div className="text-3xl font-bold">
                {user.score != null ? formatScore(user.score) : '—'}
              </div>
              <div className="text-sm text-muted-foreground">
                {user.place != null ? `#${user.place}` : 'Unranked'}
              </div>
            </div>
          </div>
        </CardContent>
